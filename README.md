...
Check that `git status` does **not** list files under `Telecommunications - SMS, Call, Internet - MI/`. Then:

```bash
git commit -m "Add notebook, README, requirements, processed caches"
git remote add origin https://github.com/YOUR_USER/YOUR_REPO.git
git push -u origin main
```

If large files were added before `.gitignore`, remove them from Git (keeps files on disk):
